...
class MetricsCollector(Microservice):
    ...
    Его задача -- его задача, по запросу от Observer'a собирать метрики из Prometheus и отправлять их в кэш
    '''

    def __init__(self, event_queue: Queue, writers: Dict[str, KafkaEventWriter], prometheus_url: str, redis_port: int):
        '''
        Инициализация класса:
        - `prometheus_url` - url Prometheus сервера.
        - `redis_port` - url Prometheus сервера.
        Поля класса:
        - `self.prometheus_url` - url Prometheus сервера.
        '''
        requests = ['sum(rate(node_cpu_seconds_total{mode!="idle", job="scaling_target"}[5m]))/count(rate(node_cpu_seconds_total{mode!="idle", job="scaling_target"}[5m]) > bool 0.05)',
                    '1 - ((avg(avg_over_time(node_memory_MemFree_bytes{job="scaling_target"}[5m]))+avg(avg_over_time(node_memory_Cached_bytes{job="scaling_target"}[5m]))+avg(avg_over_time(node_memory_Buffers_bytes{job="scaling_target"}[5m])))/avg(avg_over_time(node_memory_MemTotal_bytes{job="scaling_target"}[5m])))',
    ...
    def handle_event_get_metrics(self):
        ...
        for request in self.requests:
            ...
            metric_values = [r['value'] for r in result]
    ...
    def save_metrics_to_redis(self, metrics: Metrics):
        ...
        redis_client = redis.Redis(host='localhost', port = self.redis_port, db=0)
